...
;(function($) {
  ...
  $.fn.dialog = function(options) {
    ...
    var create = function() {
      ...
      var msghtml
        = ''
        + '<div class="dialog modal modal-wide fade" tabindex="-1" role="dialog">'
    ...
    var close = function(destroy) {
      ...
      $msgbox.modal('hide').on('hidden.bs.modal', function() {
          ...
          //stackoverflow.com/questions/5783280/check-if-div-with-certain-class-name-exists
          if ($('.modal').length) {
...
//    $('.dialog.modal.draggable>.modal-dialog').draggable({
//	    cursor: 'move',
//	    handle: '.modal-header'
//    });
//
//    $('.dialog.modal.draggable>.modal-dialog>.modal-content>.modal-header').css('cursor', 'move');
    return self;
  };
...
(function($) {
  ...
  $.fn.datagrid = function(method, options) {
    ...
    if (method == "insertRow") {
      ...
      bindRows($row);
      $tar.length ? $tar.before($row) : $("tbody", $this).append($row);
